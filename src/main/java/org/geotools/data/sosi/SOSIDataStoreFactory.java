...
public class SOSIDataStoreFactory implements DataStoreFactorySpi {
	...
	public static final Param FILE_PARAM = new Param("file", File.class, "SOSI file", true, null,
			new KVP(Param.EXT, "sos")); // KVP is a helper for defining common
										// params

	// An open ended map of all sorts of parameters for using the data store.
	...
	public synchronized boolean isAvailable() {
		if (isAvailable == null) {
			try {
				// We depend on this lib, so it has to be in the path. (Normally
				// we would add it to deps or bundle it)
